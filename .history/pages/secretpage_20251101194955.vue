...
const moonIllumination = ref(0.5); // Start with half moon for testing
...
// Calculate real moon phase with proper shape simulation
const calculateMoonPhase = () => {
  const now = new Date();
  const startOfCycle = new Date("2024-01-01");
  const daysInCycle = 29.53;
  const daysSinceStart = (now - startOfCycle) / (1000 * 60 * 60 * 24);
  const cyclePosition = (daysSinceStart % daysInCycle) / daysInCycle;

  // Determine phase name and set illumination
  let phaseName;
...
// Calculate moon phase mask style based on illumination
const moonPhaseMaskStyle = computed(() => {
  ...
  if (illumination === 0) {
    ...
    // Crescent phases
    const visiblePercentage = illumination * 2; // Convert to 0-1 scale for crescent
    const inset = 50 - visiblePercentage * 50;
    clipPath = `inset(0 ${inset}% 0 0)`;
  } else {
    // Gibbous phases
    const darkPercentage = (1 - illumination) * 2; // Convert to 0-1 scale for gibbous
    const inset = 50 - darkPercentage * 50;
    clipPath = `inset(0 0 0 ${inset}%)`;
  }

  return {
    clipPath: clipPath,
    opacity: illumination === 1 ? 0 : 0.9, // Hide mask for full moon
  };
});
...
// Moon style based on phase
const moonStyle = computed(() => ({
  boxShadow: `0 0 ${20 + moonIllumination.value * 30}px ${
    5 + moonIllumination.value * 10
  }px rgba(255, 255, 200, ${0.1 + moonIllumination.value * 0.2})`,
}));
...
const cycleMoonPhase = () => {
  // Cycle through different illumination values to test all phases
  const phases = [0, 0.25, 0.5, 0.75, 1, 0.75, 0.5, 0.25];
  const currentIndex = phases.findIndex(
    (phase) => Math.abs(phase - moonIllumination.value) < 0.1
  );
  moonIllumination.value = phases[(currentIndex + 1) % phases.length];

  // Update phase name based on illumination
  if (moonIllumination.value === 0) moonPhaseName.value = "New Moon";
  else if (moonIllumination.value === 0.25)
    moonPhaseName.value = "Waxing Crescent";
  else if (moonIllumination.value === 0.5)
    moonPhaseName.value = "First Quarter";
  else if (moonIllumination.value === 0.75)
    moonPhaseName.value = "Waxing Gibbous";
  else if (moonIllumination.value === 1) moonPhaseName.value = "Full Moon";
