...
function initFluidSimulation() {
  ...
  // Better WebGL context creation with fallbacks
  ...
  // More conservative config for mobile
  const isMobile =
  ...
  let config = {
    ...
    DENSITY_DISSIPATION: isMobile ? 0.96 : 0.98,
    VELOCITY_DISSIPATION: isMobile ? 0.97 : 0.99,
    PRESSURE_DISSIPATION: 0.8,
    PRESSURE_ITERATIONS: isMobile ? 10 : 20,
    CURL: isMobile ? 15 : 25,
    SPLAT_RADIUS: isMobile ? 0.003 : 0.005,
  };
  ...
  function getWebGLContext(canvas) {
    ...
    if (!gl) {
      try {
        ...
        if (gl) console.log("Using WebGL1 context");
      } catch (e) {
        console.error("All WebGL contexts failed:", e);
    ...
    // Get extensions with better error handling
    let halfFloat = null;
    ...
    try {
      if (gl instanceof WebGL2RenderingContext) {
        ...
        // Enable important WebGL2 extensions
      ...
      console.warn("Error getting extensions:", e);
    }

    // Use basic formats as fallback
...
`
  ...
  function initFramebuffers() {
    ...
    textureWidth = Math.max(64, textureWidth);
    textureHeight = Math.max(64, textureHeight);
  ...
  function createFBO(texId, w, h, internalFormat, format, type, param) {
    ...
    try {
      ...
      console.warn("High precision texture failed, using fallback format");
  ...
  if (!isMobile) {
    ...
    multipleSplats(parseInt(Math.random() * 5) + 2); // Fewer splats on mobile
  }
  ...
  canvasEl.addEventListener("mousemove", (e) => {
    if (isTouching) return; // Prevent mouse events during touch
  ...
  canvasEl.addEventListener(
    ...
    (e) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        pointer.dx = (touches[i].pageX - pointer.x) * 8.0; // Reduced sensitivity for mobile
