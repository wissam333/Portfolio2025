<template>
  <div class="seasonal-container" :class="currentSeason">
    ...
    >
      ...
      <!-- CHANGED: Use moon-phase-mask instead of moon-phase-overlay -->
...
// IMPROVED: More accurate moon phase calculation using Julian Date
const calculateJulianDate = (date = new Date()) => {
  return date.getTime() / 86400000 + 2440587.5;
};

// IMPROVED: Calculate moon phase with proper astronomical formula
const calculateMoonPhase = () => {
  const now = new Date();
  const jd = calculateJulianDate(now);

  // Calculate days since known new moon (January 11, 2024)
  const knownNewMoonJD = calculateJulianDate(new Date("2024-01-11"));
  const daysSinceNewMoon = (jd - knownNewMoonJD) % 29.530588853;

  // Calculate phase angle (0 to 360 degrees)
  const phaseAngle = (daysSinceNewMoon / 29.530588853) * 360;

  // Calculate illumination (0 to 1)
  moonIllumination.value = (1 - Math.cos((phaseAngle * Math.PI) / 180)) / 2;

  // Determine phase name based on days since new moon
  let phaseName;
...
// IMPROVED: More realistic moon phase mask using ellipse for crescent shapes
const moonPhaseMaskStyle = computed(() => {
  ...
  if (illumination === 0) {
    ...
    const shadowWidth = (1 - illumination * 2) * 100;
    clipPath = `ellipse(100% 100% at ${100 - shadowWidth / 2}% 50%)`;
  } else {
    // Waning phases (left side illuminated)
    const shadowWidth = (illumination * 2 - 1) * 100;
    clipPath = `ellipse(100% 100% at ${shadowWidth / 2}% 50%)`;
  }
...
// NEW: Calculate moon age in days
const moonAge = computed(() => {
  const now = new Date();
  const jd = calculateJulianDate(now);
  const knownNewMoonJD = calculateJulianDate(new Date("2024-01-11"));
  return (jd - knownNewMoonJD) % 29.530588853;
});

// NEW: Calculate moon position (simple simulation)
const moonPosition = computed(() => {
  const age = moonAge.value;
  // Simulate moon moving across sky (0% to 100%)
  const position = (age / 29.53) * 100;
  return Math.min(100, Math.max(0, position));
});

// IMPROVED: Moon style with dynamic positioning
const moonStyle = computed(() => ({
  boxShadow: `0 0 ${20 + moonIllumination.value * 40}px ${
    8 + moonIllumination.value * 15
  }px rgba(255, 255, 200, ${0.15 + moonIllumination.value * 0.3})`,
  top: `${10 + Math.sin(moonPosition.value * 0.0628) * 30}%`, // Moves up/down
  right: `${10 + (100 - moonPosition.value) * 0.8}%`, // Moves left/right
  opacity: 0.3 + moonIllumination.value * 0.7, // Brighter when more illuminated
  transform: `scale(${0.8 + moonIllumination.value * 0.4})`, // Larger when full
}));
...
// Initialize seasonal elements (keep your existing functions)
...
onMounted(() => {
  ...
  setInterval(() => {
    moonPhaseName.value = calculateMoonPhase();
    moonPhaseClass.value = moonPhaseName.value.toLowerCase().replace(" ", "-");
  }, 30000); // Update every 30 seconds for smoother transitions
});
