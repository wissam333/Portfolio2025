<template>
  <div class="text-6xl font-bold" :class="props.class">
    ...
    >
      <!-- Ambient sparkles with reduced count and conditional rendering -->
      <div class="stars" v-if="shouldShowAmbient">
        <template v-for="sparkle in visibleSparkles" :key="sparkle.id">
          <Motion
      ...
      <!-- Burst stars with limited concurrent particles -->
      <div class="burst-layer">
        <template v-for="b in visibleBursts" :key="b.id">
          <Motion
            as="svg"
...
const props = withDefaults(defineProps<Props>(), {
  sparklesCount: 5, // Reduced default count
...
const MAX_CONCURRENT_BURSTS = 15; // Limit burst particles
const AMBIENT_SPARKLE_COUNT = Math.min(props.sparklesCount, 8); // Cap ambient sparkles
...
const visibleBursts = computed(() =>
  bursts.value.slice(-MAX_CONCURRENT_BURSTS)
); // Only show recent bursts
...
function initializeStars() {
  sparkles.value = Array.from({ length: AMBIENT_SPARKLE_COUNT }, generateStar);
}
...
function processMouseMove(e: MouseEvent) {
  ...
  if (velocity > 3 && bursts.value.length < MAX_CONCURRENT_BURSTS) {
...
function handleTouchMove(e: TouchEvent) {
  if (
    !isTouching.value ||
    !containerRef.value ||
    bursts.value.length >= MAX_CONCURRENT_BURSTS
  )
    return;
...
function spawnBurst(
  ...
  const count = Math.min(3, MAX_CONCURRENT_BURSTS - bursts.value.length);
  const createdIds: string[] = [];

  for (let i = 0; i < count; i++) {
    const speedFactor = 80; // Reduced for performance
    ...
    const yDelta = Math.sin(angle) * distance + 20 * Math.random();

    const duration = 0.4 + Math.random() * 0.8; // Reduced duration
    ...
    const p: BurstParticle = {
      ...
      rotateEnd: Math.random() * 360, // Reduced rotation
      scale: 0.8 + Math.random() * 0.8, // Reduced scale variation
      duration,
    };
  ...
  // Auto-cleanup bursts after max duration
...
function removeBurst(id: string) {
...
onMounted(() => {
  ...
  // Use slower interval when not visible
  ambientInterval = window.setInterval(
    updateStars,
    shouldShowAmbient.value ? 100 : 1000
  );
