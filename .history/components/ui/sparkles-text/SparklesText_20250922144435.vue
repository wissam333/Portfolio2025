...
function handleMouseMove(e: MouseEvent) {
  ...
  // threshold for burst
  if (velocity > 0.5) {
    spawnBurst(offsetX, offsetY, vx, vy);
  }
...
function spawnBurst(x: number, y: number, vx: number, vy: number) {
  const count = 6;
  const createdIds: string[] = [];

  for (let i = 0; i < count; i++) {
    // convert velocity to particle delta
    const speedFactor = 200; // tune for distance scaling
    const spreadAngle = (Math.random() * 60 - 30) * (Math.PI / 180); // ±30 deg spread
    ...
    const yDelta = Math.sin(angle) * distance + 20 * Math.random(); // slight downward curve
