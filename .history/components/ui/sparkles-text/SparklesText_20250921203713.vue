...
function generateStar(): Sparkle {
  const starX = `${Math.random() * 140 - 20}%`; // -20% to 120%
  const starY = `${Math.random() * 100 - 50}%`; // -50% to 50%
...
.stars {
  position: absolute;
  top: -50%; /* allow stars to go above and below the text */
  left: -20%;
  width: 140%; /* extend a bit horizontally so stars aren’t cramped */
  height: 200%; /* extend vertically so stars float around the text */
  pointer-events: none;
}
</style>
