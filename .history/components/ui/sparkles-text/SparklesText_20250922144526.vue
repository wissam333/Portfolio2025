<template>
  <div class="text-6xl font-bold" :class="props.class">
    ...
    >
      <!-- ambient sparkles (your original behavior) -->
      <div class="stars">
      ...
      <!-- burst layer (fire particles spawned on fast horizontal movement) -->
...
interface AmbientSparkle {
  id: string;
  x: string; // percentage string like "42.3%"
  y: string; // percentage string like "20%"
  color: string;
  delay: number;
  scale: number;
  lifespan: number;
...
interface BurstParticle {
  ...
  xStart: number; // px
  yStart: number; // px
  xDelta: number; // px to animate by
  yDelta: number; // px to animate by
  color: string;
  rotate: number;
  rotateEnd: number;
  scale: number;
...
// --- Burst logic (fire particles on fast horizontal movement) ---
...
function handleMouseMove(e: MouseEvent) {
  ...
  // threshold — tune this value if bursts are too sensitive
  const threshold = 0.6;
...
function spawnBurst(offsetX: number, offsetY: number, velocity: number) {
  ...
  for (let i = 0; i < count; i++) {
    const baseAngle = velocity > 0 ? 0 : 180; // right or left
    const angle = baseAngle + (Math.random() * 40 - 20); // spread
    const rad = (angle * Math.PI) / 180;
    const distance = 60 + Math.random() * 60;
    const xEnd = offsetX + Math.cos(rad) * distance;
    const yEnd = offsetY + Math.sin(rad) * distance - (10 + Math.random() * 20); // slight upward bias
    const xDelta = Math.round(xEnd - offsetX);
    const yDelta = Math.round(yEnd - offsetY);
    const duration = 0.45 + Math.random() * 0.5;
    ...
    const p: BurstParticle = {
      ...
      rotateEnd: Math.round(Math.random() * 720),
  ...
  // remove these particles after their animation completes
...
.stars {
  ...
  width: 100%;
  height: 100%;
  pointer-events: none; /* keeps stars from blocking text */
}

/* overlay for burst particles — kept inset so x/y px coords map to span top-left */
.burst-layer {
  position: absolute;
  inset: 0;
  pointer-events: none;
