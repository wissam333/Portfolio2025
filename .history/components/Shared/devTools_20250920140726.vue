...
const name = "Wissam Najjom";
let i = 0;

function typeName() {
  if (i < name.length) {
    // Print next character without newline
    process.stdout
      ? process.stdout.write(name[i])
      : console.log(name.slice(0, i + 1)); // fallback for browser console
    i++;
    setTimeout(typeName, 150); // adjust typing speed (ms)
  } else {
