...
Keep exploring, amazing stuff awaits! 🚀
`;
let display = "";
let i = 0;
let cursor = "|";
...
function blinkCursor() {
  ...
  setInterval(() => {
    console.clear();
    console.log(text + (showCursor ? cursor : " "));
    showCursor = !showCursor;
  }, 500);
}

// -------------------- Interactive console message --------------------
function showInteractiveConsole() {
  // Make the function globally accessible
  window.secretFunction = () => {
    alert("🎉 Secret Easter Egg Activated!");
    particlesRef.value?.triggerParticles(100);
  };

  console.log(
    "%c🌈 Type secretFunction() in console to activate something cool! 🎨",
    "color: white; background: linear-gradient(90deg,#ff6ec4,#7873f5); font-size:16px; font-weight:bold; padding:5px 10px; border-radius:5px;"
  );

  console.log({
    clickMe: window.secretFunction,
  });
}
</script>
