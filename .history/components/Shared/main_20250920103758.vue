<template>
  <div :class="['egg-counter', positionClass]">
    <div class="counter-inner">
      <!-- Header -->
      <div class="header">
        <svg class="icon" viewBox="0 0 24 24"><path d="M12 2a10 10 0 1010 10A10 10 0 0012 2zm1 15h-2v-2h2zm0-4h-2V7h2z"/></svg>
        <span class="title">Easter Eggs</span>
      </div>

      <!-- Count -->
      <div class="count">
        <span class="num">{{ eggs.length }}</span>
        <span v-if="typeof total === 'number'" class="total">/ {{ total }}</span>
      </div>

      <!-- Progress (optional) -->
      <div v-if="typeof total === 'number'" class="progress">
...
const props = defineProps({
  eggs: { type: Array, required: true }, // your state array
  total: { type: Number, required: false }, // total eggs (optional)
  position: { type: String, default: 'bottom-left' } // UI position
})

const percent = computed(() => {
  if (typeof props.total !== 'number' || props.total <= 0) return 0
  return Math.min(100, Math.round((props.eggs.length / props.total) * 100))
})
...
const positionClass = computed(() => {
  switch (props.position) {
    ...
    default: return 'pos-bottom-left'
  }
})
</script>
...
.counter-inner {
  ...
  box-shadow: 0 6px 20px rgba(0,0,0,0.5),
              0 0 20px rgba(140,180,255,0.15);
  color: #e6f0ff;
}
