...
const onStarHover = (event) => {
  ...
  event.target.classList.add("star-glow");
};

const onStarLeave = (event) => {
  // Keep the cursor glowing for a moment after leaving the star
  setTimeout(() => {
    isCursorGlowing.value = false;
  }, 1000);

  // Remove glow effect from the star
  event.target.classList.remove("star-glow");
};
...
const checkHiddenElements = () => {
  ...
  hiddenItems.forEach((item, index) => {
    ...
    if (distance < cursorRadius && isCursorGlowing.value) {
