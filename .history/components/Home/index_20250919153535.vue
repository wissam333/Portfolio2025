...
function createStars(count = 30) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const animationDuration = (Math.random() * 10 + 3).toFixed(2) + "s";
...
const onStarEnter = (index) => {
  // pause & glow that star
  hoveredStar.value = index;

  // start the glow cursor phase
  ...
  // after glowDuration -> switch to flashlight
  const glowDuration = 1500;
  glowTimeout = setTimeout(() => {
    isCursorGlowing.value = "flashlight";

    // let the hovered star finish its fade and resume after a short time
    setTimeout(() => {
      if (hoveredStar.value === index) hoveredStar.value = null;
    }, 350);
    glowTimeout = null;
  }, glowDuration);
};

const onStarLeave = (index) => {
  // if leaving early, cancel the planned flashlight switch
  if (glowTimeout) {
    clearTimeout(glowTimeout);
    glowTimeout = null;
  }
...
const checkHiddenElements = () => {
  if (isCursorGlowing.value !== "flashlight") {
    // clear reveals when not in flashlight mode
    revealedElements.value = [];
    return;
  }

  const hiddenItems = document.querySelectorAll(".hidden-item");
  const cursorRadius = 100; // flashlight radius in px (tweak if needed)
  const newlyRevealed = [];
  ...
  hiddenItems.forEach((item, index) => {
    ...
    const distance = Math.hypot(
      centerX - cursorX.value,
      centerY - cursorY.value
    );
  ...
  revealedElements.value = newlyRevealed;
};
...
/* Hide native cursor but show our custom one */
.home {
  position: relative;
  overflow: hidden;
  height: 100vh;
...
/* wrapper positions the star horizontally (left). top is handled by star animation */
.star-wrapper {
  position: absolute;
  top: 0;
...
/* .star holds the falling animation (transform). pausing this won't affect child scaling */
.star {
  position: absolute;
  top: -20px;
  left: 0;
...
/* Pause only the hovered star */
.star.paused {
  animation-play-state: paused;
}

/* visual dot + trail (child). scaling this won't replace parent's transform */
.star-inner {
  position: absolute;
  top: 0;
  left: 0;
...
.star-inner::before {
  ...
  opacity: 0;
  animation: trailFade var(--animationDuration, 6s) linear infinite
    var(--animationDelay, 0s);
  pointer-events: none;
}

/* Hover state for the inner dot (scales + glow) */
.star-inner.hovered {
  transform: scale(3);
  opacity: 1 !important;
...
/* falling move (applies to .star element) */
@keyframes fall {
  0% {
...
.glow-cursor {
  ...
  /* base cursor visible as a small dot */
  width: 12px;
  height: 12px;
  border-radius: 50%;
...
.glow-cursor.glow {
  ...
  background: radial-gradient(
    ...
    rgba(79, 192, 141, 0.35) 65%,
    transparent 100%
  );
  box-shadow: 0 0 30px #4fc08d, 0 0 60px rgba(79, 192, 141, 0.5);
  mix-blend-mode: screen;
}

/* flashlight - bigger soft circle that reveals things in a radius */
...
/* Hidden elements that appear when in flashlight radius */
.hidden-elements {
  position: absolute;
  top: 0;
  left: 0;
...
/* ---------- Content, text animations and UI (unchanged except minor) ---------- */
.content {
  position: relative;
  z-index: 2;
  text-align: center;
...
/* Buttons and scroll indicator (keep previous behavior) */
.button-group {
  display: flex;
  gap: 1.5rem;
  justify-content: center;
