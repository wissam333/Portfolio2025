...
const checkMoonProximity = () => {
  ...
  if (distance < MOON_RADIUS + 100) {
    ...
    if (length > 0) {
      ...
      const gravityStrength = MOON_GRAVITY * (1 - (distance / (MOON_RADIUS + 100)));
...
const checkMoonLanding = () => {
  ...
  if (distance < MOON_RADIUS) {
    ...
    // Snap astronaut to moon center
    astronautPosition.value = {
      x: MOON_CENTER.x - 25, // Adjust for astronaut center
      y: MOON_CENTER.y - 50
    };
...
useRafFn(() => {
  ...
  if (Math.random() < 0.01) { // 1% chance per frame
    console.log('Astronaut position:', astronautPosition.value);
  }
...
// Temporary debug function - remove when working
const debugMoonPosition = () => {
  const debugElement = document.getElementById('moon-debug');
  ...
  debugDiv.id = 'moon-debug';
  debugDiv.style.cssText = `
    position: absolute;
    left: ${MOON_CENTER.x - moonRect.width/2}px;
    top: ${MOON_CENTER.y - moonRect.height/2}px;
    width: ${moonRect.width}px;
    height: ${moonRect.height}px;
    border: 2px solid red;
    border-radius: 50%;
    pointer-events: none;
    z-index: 1000;
  `;
  container.value.appendChild(debugDiv);
};

// Call this in onMounted and after drag operations
</script>
