    ...
    <div class="hidden-elements-container" id="hiddenPage">
      <div class="hidden-item" style="top: 10px; inset-inline-start: 10px">
        <img
          ...
          alt="got Me"
          class="gotMe"
          @mouseenter="useEasterEggs().value.push(2)"
        />
      </div>
    </div>
...
.gotMe.exposed {
  transform: scale(1.1);
  box-shadow: 0 0 20px $primary-color, 0 0 40px rgba($primary-color, 0.5);
  border: 2px solid $primary-color;
  animation: pulseFound 1.2s ease-in-out infinite alternate;
}

@keyframes pulseFound {
  0% {
    transform: scale(1.05);
    box-shadow: 0 0 15px $primary-color, 0 0 30px rgba($primary-color, 0.4);
  }
  100% {
    transform: scale(1.15);
    box-shadow: 0 0 25px $primary-color, 0 0 50px rgba($primary-color, 0.6);
  }
}
