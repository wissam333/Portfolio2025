...
const astronautPosition = ref({ x: 30, y: 20 });
...
const onDrag = (e) => {
  ...
  const container = document.querySelector(".night-illustration");
  const containerRect = container.getBoundingClientRect();

  let x =
    ((e.clientX - containerRect.left - dragOffset.value.x) /
      containerRect.width) *
    100;
  let y =
    ((e.clientY - containerRect.top - dragOffset.value.y) /
      containerRect.height) *
    100;

  // Constrain to container bounds with larger range
  x = Math.max(-20, Math.min(120, x));
  y = Math.max(-30, Math.min(130, y));

  astronautPosition.value = { x, y };
};
...
// Update getStarStyle function to include larger range
const getStarStyle = (n) => {
  const size = Math.random() * 3 + 1;
  const opacity = Math.random() * 0.7 + 0.3;
  const twinkleDelay = Math.random() * 5;
  return {
    top: `${Math.random() * 120 - 10}%`, // Increased range
    left: `${Math.random() * 120 - 10}%`, // Increased range
    width: `${size}px`,
    height: `${size}px`,
    opacity: opacity,
