...
const createSnowflake = (x = null, isBlizzardFlake = false) => {
  ...
  return {
    ...
    speed: isBlizzardFlake
      ? 2 + Math.random() * 3 // Faster in blizzard
      : 0.5 + Math.random() * 1.5, // Normal speed
    drift: (Math.random() - 0.5) * (isBlizzardFlake ? 2 : 0.8), // More drift in blizzard
    rotation: Math.random() * 360,
    rotationSpeed: (Math.random() - 0.5) * (isBlizzardFlake ? 10 : 4),
    isBlizzardFlake,
    style: {},
  };
};
...
const animateSnowflakes = () => {
  ...
  activeSnowflakes.value = activeSnowflakes.value.filter((snowflake) => {
    // Update position
    snowflake.y += snowflake.speed * 0.05;
    snowflake.x += snowflake.drift * (isBlizzard.value ? 1.5 : 1);
