    ...
    <div class="home">
      ...
      >
        <div class="hidden-item" style="top: 10px; inset-inline-end: 10px">
          ...
          <CelebrationParticles ref="celebration" />
        </div>
      </div>
...
const animateCursor = () => {
  ...
  if (
    isCursorGlowing.value ||
    Math.abs(targetX - cursorX.value) > 1 ||
    Math.abs(targetY - cursorY.value) > 1
  ) {
...
watch(isCursorGlowing, (val) => {
  if (val && !rafId) rafId = requestAnimationFrame(animateCursor);
});

// ── Easter egg: hover gotMe gif while flashlight is on ────────────────────────
let gifCounted = false;

const checkGifHover = (e) => {
  if (gifCounted || isCursorGlowing.value !== "flashlight") return;
  const gif = document.querySelector(".gotMe");
  if (!gif) return;
  const { left, right, top, bottom } = gif.getBoundingClientRect();
  if (
    e.clientX >= left &&
    e.clientX <= right &&
    e.clientY >= top &&
...
// ── Intro text animations (CSS-driven via refs, no querySelector) ─────────────
