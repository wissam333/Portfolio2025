...
const updateCursor = (e) => {
  cursorX.value = e.clientX || e.touches[0].clientX;
  cursorY.value = e.clientY || e.touches[0].clientY;

  document.documentElement.style.setProperty('--cursorX', cursorX.value + 'px');
  document.documentElement.style.setProperty('--cursorY', cursorY.value + 'px');
};
...
.home::before {
  ...
  inset: 0;
  pointer-events: none;
  z-index: 999; // above stars, below content
  background: radial-gradient(
    circle 15vmax at var(--cursorX, 50vw) var(--cursorY, 50vh),
    rgba(0, 0, 0, 0) 0%,
    rgba(0, 0, 0, 0.5) 50%,
    rgba(0, 0, 0, 0.95) 100%
  );
