...
.hidden-elements-container {
  position: absolute;
  inset: 0;
  pointer-events: none; // so the main content is interactive
  z-index: 5;

  --cursorX: 50vw;
  --cursorY: 50vh;

  /* Dark overlay by default, except the flashlight area */
  &::before {
    content: "";
    position: absolute;
    inset: 0;
  ...
  .hidden-item {
    position: absolute;
    opacity: 1; // always visible, but under dark overlay
    transform: scale(1);
    padding: 10px 15px;
    border-radius: 20px;
    background: rgba(0, 0, 0, 0.7);
