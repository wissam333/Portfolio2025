...
// Activate glow cursor when hovering over a star
const activateGlowCursor = () => {
  isCursorGlowing.value = true;
  
  // Set a timeout to automatically deactivate the cursor after 5 seconds
  setTimeout(() => {
...
const checkHiddenElements = () => {
  if (!isCursorGlowing.value) return;
  
  const hiddenItems = document.querySelectorAll('.hidden-item');
  const cursorRadius = 50; // Size of the glow effect
...
.glow-cursor {
  ...
  &.active {
    ...
    width: 100px;
    height: 100px;
    background: radial-gradient(circle, rgba(255, 255, 255, 0.8) 0%, rgba(79, 192, 141, 0.4) 70%, transparent 100%);
