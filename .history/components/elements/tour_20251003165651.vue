...
const overlayMaskStyle = computed(() => {
  if (!highlightStyle.value) return {};

  const { top, left, width, height } = highlightStyle.value;

  // parse px back to numbers
  ...
  return {
    WebkitMask: `radial-gradient(circle at ${x + w / 2}px ${y + h / 2}px,
                  transparent ${Math.max(w, h) / 2}px,
                  black ${Math.max(w, h) / 2 + 1}px)`,
    mask: `radial-gradient(circle at ${x + w / 2}px ${y + h / 2}px,
                  transparent ${Math.max(w, h) / 2}px,
                  black ${Math.max(w, h) / 2 + 1}px)`,
  };
});
